...
- `api/routes/jobs.py` — `GET /jobs/<id>` reads Job + Song rows from DB; `DELETE /jobs/<id>` manually deletes a job + songs (anyone when social login is off; with `USE_SOCIAL_LOGIN` only admins via `JOB_ADMIN_GROUPS` or the requesting user themselves); job responses carry a `can_delete` flag for the frontend
- `api/routes/stats.py` — `GET /stats` aggregates job counts (respecting user/group visibility), library size (albums/playlists/artists/tracks), the album genre distribution, and the top social-login users (by job count, capped by `TOP_USERS_LIMIT` default 5, with per-user track totals) for the frontend Stats tab; lazily backfills missing album genres (bounded by `GENRE_BACKFILL_LIMIT`, default 3, per request) via the iTunes lookup
...
- `tasks/download.py` — Celery tasks wrap `download_bulk` with `billiard.Pool`. Updates task state per song for progress tracking. Records successfully downloaded albums/playlists into `available_albums` with their `browse_id`; album downloads also record the resolved `genre` (playlists stay NULL).
...
- `available_albums.genre` is stored from download time onward; albums predating the column get their genre lazily backfilled by `GET /stats` (capped per request) — index scans (S3/Navidrome) never clobber it because the upsert only writes genre when provided
